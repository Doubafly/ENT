...
export async function PUT(request: Request) {
  try {
    const { id_filiere, nom, description, niveau, montant_annuel, id_annexe } = await request.json();
    if (!id_filiere || !nom || !description || !niveau || !montant_annuel || !id_annexe) {
    ...
    return new Response(JSON.stringify({ message: "succes", filiere }), { status: 200 });
  } catch (e) {
...
export async function GET(request: Request, { params }: { params: { id: string } }) {
    try {
        ...
        if (!filieres || filieres.length === 0) {
            return new Response(JSON.stringify({ message: "Aucune filiere trouvée" }), { status: 404 });
        }
