...
export default function UserProfile({ user }: UserProfileProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const reponse = await fetch("/api/annonce").then((res) => res.json());
        console.log(reponse, "resyuh");

        setNotificationCount(reponse.annonces.length);
      } catch (err) {
  ...
  return (
    <div className="flex items-center p-3 rounded-xl mb-3">
      <div className="flex space-x-2">
        {links
          .filter((items) => items.title === "Annonces")
          .map((items) => {
            return (
