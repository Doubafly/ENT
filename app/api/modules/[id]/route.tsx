...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!id_module || !nom || !description) {
      return new NextResponse(JSON.stringify({ message: "Paramètres manquants" }), {
        status: 400,
      });
    }
    ...
    return new NextResponse(JSON.stringify({ message: "succes", module }), {
      status: 200,
    });
  } catch (error) {
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    return new NextResponse(JSON.stringify({ message: "succes" }), { status: 200 });
  } catch (error) {
    return new NextResponse(
      JSON.stringify({ message: "Une erreur est survenue" }),
      { status: 500 }
