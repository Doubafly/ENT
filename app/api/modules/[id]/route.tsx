...
export async function PUT(request: Request) {
    try {
        ...
        return new Response(JSON.stringify({ message: "succes", module }), { status: 200 });
    } catch (error) {
...
export async function DELETE(request: Request) {    
    try {
        ...
        return new Response(JSON.stringify({ message: "Une erreur est survenue" }), { status:
500 });
    }
}   
...
export async function GET(request: Request, { params }: { params: { id: string } }) {
    try {
        ...
        return new Response(JSON.stringify({ message: "succes", modules }), { status: 200 });
    } catch (error) {
        return new Response(JSON.stringify({ message: "Une erreur est survenue" }), { status: 500 });
    }
}